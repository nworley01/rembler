...
CLASS_MAP = {
    "WAKE": 0, "W": 0, "WA": 0,
    "NREM": 1, "N": 1, "SWS": 1, "N2": 1, "N3": 1,  # rodents often just SWS/NREM
    "REM":  2, "R": 2, "P": 2,  # P for paradoxical
}
UNKNOWN = -1
...
def labels_from_csv(csv_path: str, epoch_sec: int, total_sec: float | None=None) -> np.ndarray:
    ...
    if {'epoch','stage'}.issubset(df.columns):
        n_epochs = int(df['epoch'].max() + 1) if total_sec is None else int(np.floor(total_sec / epoch_sec))
        y = np.full(n_epochs, UNKNOWN, dtype=np.int16)
        ...
        ann = list(df[['onset_sec','duration_sec','stage']].itertuples(index=False, name=None))

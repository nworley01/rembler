...
def _pick_channels(raw: mne.io.BaseRaw, wanted: list[str] | None) -> list[int]:
    ...
    for w in wanted:
        matches = (
            ...
            if any(x in w for x in r".*[]()|^$")
            else mne.pick_channels(raw.ch_names, include=[w], ordered=False)
        )
...
def load_edf(path: str, cfg: EDFLoadConfig) -> Recording:
    ...
    eeg_ix = (
        _pick_channels(raw, cfg.eeg_channels)
        if cfg.eeg_channels
        else mne.pick_types(raw, eeg=True)
    )
    emg_ix = (
        _pick_channels(raw, cfg.emg_channels)
        if cfg.emg_channels
        else mne.pick_types(raw, emg=True)
    )

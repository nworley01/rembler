...
from rembler.utils import io_utils, sleep_utils
...
def process_edf_files(edf_files):
    for file in edf_files:
        df = sleep_utils.extract_sleep_stages_from_edf(os.path.join(EDF_DIR, file))
        sleep_utils.save_sleep_stages_datatable(
            df, file.replace(".edf", ".csv"), OUT_DIR
        )
        print(f"Processed {file} and saved to {OUT_DIR}")


if __name__ == "__main__":
    process_edf_files(edf_files)
